...
public:
	...
	static IndexedTriangleList<V> Make()
	{
		...
		vertices[0].pos = { side, side, side }; // 7
		vertices[0].pos = { side, -side, -side }; // 1
		vertices[0].pos = { -side, side, -side }; // 2
		vertices[0].pos = { side, side, -side }; // 3
		vertices[0].pos = { -side, -side, side }; // 4
		vertices[0].pos = { side, -side, side }; // 5
		vertices[0].pos = { -side, side, side }; // 6
