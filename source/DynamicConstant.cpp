...
namespace DynamicConstBuf
{
	...
	size_t LayoutElement::GetOffsetEnd() const noxnd
	{
		...
		case Struct:
		{
			const ExtraData::Struct& structData = static_cast<ExtraData::Struct&>(*pExtraData);
			return AdvanceToBoundary(structData.layoutElements.back().second.GetOffsetEnd());
		}
		case Array:
		{
			const ExtraData::Array& arrayData = static_cast<ExtraData::Array&>(*pExtraData);
			return *offset + AdvanceToBoundary(arrayData.layoutElement->GetSizeInBytes()) * arrayData.size;
		}
		default:
	...
	size_t LayoutElement::FinalizeForStruct(size_t offsetIn)
	{
		...
		for (auto& el : structData.layoutElements)
		{
